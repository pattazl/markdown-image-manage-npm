...
md-img a e:\doc\test.md
 Will analyse images in[e:\doc\test.md]
 ----error message total[3]----
d:\image1.png | not exist
d:\image2.png| not exist
d:\image3.png | not exist
...
详细参数如下
  ...
  -l, --local <path> 图片要下载的目标路径，可设置绝对路径或相对路径，默认路径[md文件名.assets]，如md文件为test.md ，则目录为 test.assets
  ...
  -o, --overwrite   修改md文件中的图片链接时不备份原文件，直接覆盖，如无参数则将备份原文件，原文件名以_localBK结尾。
...
上传md文件中的本地图片，详细参数如下
  ...
  -o, --overwrite          修改md文件中的图片链接时不备份原文件，直接覆盖，如无参数则将备份原文件，原文件名以_netBK结尾。
  -p, --remotepath <path>  图床上需要添加的远程路径，默认为空。为空时表示在图床上建立以md文件名的目录 ，以区分不同md文件的图片，用 / 或\ 时表示直接不添加目录，也可自定义其他目录名。 
...
移动md文件中的本地图片到指定目录，详细参数如下
  ...
  -o, --overwrite   修改md文件中的图片链接时不备份原文件，直接覆盖，如无参数则将备份原文件，原文件名以_moveBK结尾。
